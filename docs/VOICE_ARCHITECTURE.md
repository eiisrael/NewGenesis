...
- `VoiceStateMachine`: `IDLE`, `LISTENING`, `SPEECH_DETECTED`, `TRANSCRIBING`, `THINKING`, `PREPARING`, `SPEAKING`, `INTERRUPTING` e `ERROR`;
- `MicrophoneAudioInput` + `AdaptiveEnergyVad`: `AudioWorklet` primário, fallback explícito, pre-roll, VAD adaptativo, filtragem antialias e WAV mono 16 kHz;
...
- `AudioPlaybackController`: fila sequencial, no máximo uma preparação em voo, cancelamento imediato e fallback exclusivamente local;
- `LocalTextToSpeechEngine`: prepara, decodifica e toca Kokoro, Piper ou Chatterbox;
- `SystemTextToSpeechEngine`: usa somente voz portuguesa com `localService === true` para cobrir aquecimento sem rede;
...
| Voz do sistema | Web Speech API + voz `localService` | gerenciada pelo Windows/navegador | Piper local |

O automático prefere workers saudáveis e aquecidos. Piper é priorizado neste hardware; uma voz pt-BR local do sistema cobre o aquecimento. Kokoro continua disponível por seleção explícita ou quando aquecido e saudável. Chatterbox continua experimental e pesado.
...
- texto TTS até 2.000 caracteres; uma operação STT ativa e uma fila TTS FIFO com uma síntese por vez;
- deadline STT absoluto de 18 s (`rapid`), 30 s (`balanced`) ou 50 s (`accurate`); TTS de 28 s (Kokoro) ou 60 s (Piper/Chatterbox), e WAV TTS até 24 MiB;
- cancelamento HTTP propagado ao processo ativo; Kokoro entra em cooldown de 30 s após falha fria;
